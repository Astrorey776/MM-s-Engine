...
void PhysVehicle3D::Render()
{
	...
	CreateCarPart(vec3( 0.5, 0.8,0.8 ), { 0, 2, 1.2 }, Red).Render();
	CreateCarPart(vec3( 0.25f, 0.4f, 0.4f ), { 0.5f, 1.4f, 2.4f }, White).Render();
	CreateCarPart(vec3( 0.25f, 0.4f, 0.4f ), { -0.5f, 1.4f, 2.4f }, White).Render();
	CreateCarPart(vec3( 1.4, 1,1.5 ), { 0, 2, -0.2 }, White).Render();
	CreateCarPart(vec3( 0.1, 0.3,.3 ), { .6, 2.2, -1.5 }, Red).Render();
	CreateCarPart(vec3( 0.1, 0.3,.3 ), { -.6, 2.2, -1.5 }, Red).Render();
	CreateCarPart(vec3( 2, 0.07,.5 ), { 0, 2.35, -1.5 }, Red).Render();

}

CubeA PhysVehicle3D::CreateCarPart(const vec3 size, const vec3 position, const Color color)
{
	CubeA part(size.x, size.y, size.z);
